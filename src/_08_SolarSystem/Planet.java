...
public class Planet {
	...
	public Planet(int diameterPixels, int distanceFromSunPixels, int daysPerFullOrbit, Color color) {
		...
		this.color = color;
		this.distanceFromSun = distanceFromSunPixels;
		this.orbitalPeriod = daysPerFullOrbit;
	}

	public void draw(Graphics g, int numDays) {
		/*
		 * Local variables Reassign or remove these after creating the correct member
		 * variables
		 */
		 ...
		 */
		g.setColor(color);
		g.drawOval(SolarSystem.CENTER_X - avgDistFromSun, SolarSystem.CENTER_Y - avgDistFromSun, 2 * avgDistFromSun,
				2 * avgDistFromSun);
		...
		for (int i = 0; i < moons.size(); i++) {
			...
			if (i == 0) {
				g.drawOval(centerX - moon.distPlanetToMoon, centerY - moon.distPlanetToMoon, 2 * moon.distPlanetToMoon,
						2 * moon.distPlanetToMoon);
			}
	...
	class Moon {
		...
		public void draw(Graphics g, int days, int planetX, int planetY) {
			...
			g.fillOval(planetX - (moonDiameter / 2) + moonX, planetY - (moonDiameter / 2) + moonY, moonDiameter,
					moonDiameter);
		}
	}
}
